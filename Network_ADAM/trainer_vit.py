...
class Trainer:
    ...
    def __init__(self, network, train_loss_func, test_loss_func, optimizer, train_loader, test_loader, device,
                 noise_module = None, noise_factor: float = 1, name: str = None):
    ...
    def train(self, epoch: int, log_int = 1, lr_file: str = None, verbose=True):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            loss = self.train_loss_func(pred, target)
            ...
            if 100*batch_idx//length >= next_perc:
                ...
                print(
                    f'Train Epoch: {epoch} [{current}/{total} ({percentage:.0f}%)] Loss: {loss.item():.4f} Current LR: {current_lr:.6f}',
                    end="\r")     
        print(CLEAR, end="\r")
    ...
    def test(self, log_int=5, add_noise: bool = False, loss_file: str = None, perf_file: str = None, verbose = True):
        ...
        with torch.no_grad():  # Disable gradient computation for testing
            for batch_idx, (data, target) in enumerate(self.test_loader):
                ...
                # Add the loss between the predictions and the target values
                loss = self.test_loss_func(pred, target)
                avg_loss += loss/length
    ...
    def restore_network_state(self, network_path: str):
        state_dict = torch.load(network_path,  map_location=self.device)
    ...
    def save_figures(self, n: int, save_path: str, err_max: float = None, add_noise: bool = False):
        ...
        for idx in range(n):
            self._generate_figure(data[idx], target[idx], pred[idx], err_max=err_max)
    ...
    def _generate_figure(self, data: torch.Tensor, target: torch.Tensor, pred: torch.Tensor, err_max: float = None):
        ...
        ssim_loss_func = SSIMLoss(window_size=11).to(self.device)
        rmse_loss_func = RMSELoss().to(self.device)
        ...
        with torch.no_grad():  # Disable gradient computation for testing
            for batch_idx, (data, target) in enumerate(self.test_loader):
                ...
                if loss[arg] > threshold:
                    self._generate_figure(data[arg], target[arg], pred[arg])
                    batch_size = len(data)
                    data_idx = batch_size*batch_idx + arg
                    plt.savefig(f"{save_path}/error_figure-{data_idx+1:>04}")
                    plt.close()
