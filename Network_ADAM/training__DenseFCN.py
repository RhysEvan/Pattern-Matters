...
def densenet_train(name, idx):

    full_name = f"{name}_{idx}_DenseNet"
    ...
    network_path = f"output/trained_models/avg_pool_{full_name}.pth"
    figures_path = f"output/figures/{full_name}"
    error_fig_path = f"output/error_figures/{full_name}"
    loss_file = f"output/loss/avg_pool_{full_name}_rmse.txt"
    perf_file = f"output/perf/DenseNet_3x3_kernel.txt"
    ...
    network = DenseNetFCN(depth=10, growth_rate=12, num_classes=1, prepadding=2)
