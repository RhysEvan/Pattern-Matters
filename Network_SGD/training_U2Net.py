...
def u2net_train(name, idx):

    full_name = f"U2Net_{name}_{idx}"
    ...
    network = UNet(prepadding=2, feature_base=16, n_classes=2, apply_sigmoid=False)
    ...
    labels = torch.cat((nums, dens), dim=1)
