...
for n_periods in periods_list:
    ...
    for idx in range(start_set, end_set+1):
        ...
        nums = torch.zeros((n, height, width))
        dens = torch.zeros((n, height, width))
        ...
        for i, Z in enumerate(labels):
            ...
            nums[i] = torch.from_numpy(proj4.array-proj2.array).unsqueeze(0)
            dens[i] = torch.from_numpy(proj1.array-proj3.array).unsqueeze(0)
        ...
        nums = nums.unsqueeze(1)  # Add feature dimension
        torch.save(nums, os.path.join(data_dir, f"{name}_nums_{idx}.pth"))
        dens = dens.unsqueeze(1)  # Add feature dimension
        torch.save(dens, os.path.join(data_dir, f"{name}_dens_{idx}.pth"))
